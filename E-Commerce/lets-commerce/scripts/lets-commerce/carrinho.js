...
const reduzProdutosCarrinho = (acc, produto) => {
    const indiceProdutoAcc = acc.findIndex(buscaProdutosCarrinhoPorId(produto.id));
    ...
    for (produto of produtosUnificados){
        ...
        <div class="card">
            ...
            <div class="quantidade-produtos">
                ...
                <span class="material-icons adicao" onclick="adicionaQuantidadeProduto(${produto.id})"> add </span>
                <span><b>${produto.quantidade}</b></span>
                <span class="material-icons remocao" onclick="removeQuantidadeProduto(${produto.id})"> remove </span>
            </div>
            </div>
            <div class="acoes">
                <span class="material-icons acao" onclick="removeProdutoCarrinho(${produto.id})"> delete </span>
            </div>
        </div>
        `
    }
...
const buscaProdutosCarrinhoPorId = id => produto => produto.id === id;
...
const removeQuantidadeProduto = (id) => {
    const indiceProduto = produtosCarrinho.findIndex(buscaProdutosCarrinhoPorId(id));
...
const adicionaQuantidadeProduto = (id) => {
    const produto = produtosCarrinho.find(buscaProdutosCarrinhoPorId(id));
    produtosCarrinho.push(produto);
    atualizaListagemCarrinho();
}
